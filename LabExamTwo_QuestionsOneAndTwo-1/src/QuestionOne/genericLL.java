...
/**
 * This class represents a make-shift linklist class separate from java.util.LinkedList.
 *
 * @author Matthew Re - Clemson University
 */
public class genericLL
{
     ...
     * Note: Does not move the curr iterator down the list.
     */
    public void setCurr(Double aData)
    {
        if(curr!=null)
            curr.data = aData;
    }
     ...
     * Note: Does not move the curr iterator down the list.
     */
    public void insert(Double aData)
    {
    ...
    {
        if(curr == null)//list is already empty
            return;

        if(curr == head && curr.link == null)//current is the head and it's the only node
        {
            curr = null;
            head = null;
            prev = null;
        ...
        {
            if(curr == head)//delete the head, but the head isn't the only item
            {
                head = head.link;
                prev = null;
                return;
            }
            if(prev != null && curr.link != null)//we're looking at a node that exists mid-list
            {
                prev.link = curr.link;
                curr = prev;
                return;
            }
            if(curr.link == null)//we're at the tail.
            {
                curr = prev;
                if (curr != null) {
                    curr.link = null;
     ...
     * Note: Does not move the curr iterator down the list.
     */
    public boolean isContained(Double aData)//basically a search for the input data
    {
     ...
     */
    private ListNode findNodeWith(Double aData)
    {
        ListNode temp = head;
        while(temp != null)
        {
     ...
     *      such a node D.N.E.]
     */
    public void goToNodeWithData(Double aData)
    {
        if(head == null)
            return;
